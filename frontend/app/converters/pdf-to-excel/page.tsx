...
import { CloudUpload, Table, FileText, CheckCircle, AlertCircle } from "lucide-react";
...
export default function PdfToExcelPage() {
  ...
  const getFilenameFromContentDisposition = (cd: string | null) => {
    if (!cd) return null;
    // basic parse of: attachment; filename="file.xlsx" or filename*=UTF-8''file.xlsx
  ...
  const uploadAndDownload = async (file: File) => {
    ...
    // optional param: tables_only
    formData.append("tables_only", String(tablesOnly));

    try {
      const response = await fetch("http://127.0.0.1:8000/api/pdf-to-excel/", {
        method: "POST",
        body: formData,
      });
      ...
      let outFilename = headerName || file.name.replace(/\.pdf$/i, ".xlsx");
      ...
      <div className="max-w-2xl w-full">
        <div className="mb-6">
          ...
          <p className="mt-1 text-sm text-gray-600">
            Upload a PDF and download an Excel (.xlsx). Tables detected in the PDF become sheets; if no tables are found,
            page text will be placed into sheets. Files are sent to your backend at{" "}
            <code className="bg-gray-100 px-1 rounded">/api/pdf-to-excel/</code>.
          </p>
        </div>
        ...
        >
                ...
                <div className="mt-3 flex items-center gap-4">
                  <label className="flex items-center gap-2 text-sm">
                    <input
                      ...
                      className="rounded"
                    />
                    <span className="text-sm text-gray-600">Tables only (return error if no tables)</span>
                  </label>
                </div>
              </div>
            </div>
          ...
          <div className="mt-6 text-sm text-gray-500">
            <p>Accepted: PDF files only. Max file size: depends on your backend.</p>
          </div>
