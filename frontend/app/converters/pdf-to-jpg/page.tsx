...
import { CloudUpload, Image as ImageIcon, CheckCircle, AlertCircle } from "lucide-react";
...
export default function PdfToJpgPage() {
  ...
  const uploadAndDownload = async (file: File) => {
    ...
    formData.append("first_page_only", String(firstPageOnly));
    ...
    try {
      ...
      // prefer filename from header, fallback to original with changed ext
      const headerName = getFilenameFromContentDisposition(contentDisposition);
      let outFilename = headerName;
      if (!outFilename) {
        if (contentType.includes("image")) outFilename = file.name.replace(/\.pdf$/i, ".jpg");
        else if (contentType.includes("zip")) outFilename = file.name.replace(/\.pdf$/i, ".zip");
        else outFilename = file.name.replace(/\.pdf$/i, ".out");
      }
      ...
      <div className="max-w-2xl w-full">
        <div className="mb-6">
          ...
          <p className="mt-1 text-sm text-gray-600">
            Upload a PDF and download JPG images (single image for a single page or first page only, or a ZIP of
            page images for multi-page PDFs). Files are sent to your backend at{" "}
            <code className="bg-gray-100 px-1 rounded">/api/pdf-to-jpg/</code>.
          </p>
        </div>
        ...
        >
              ...
              <div>
                ...
                <div className="mt-3 flex items-center gap-3">
                  ...
                  <input
                    ...
                    max={600}
                    value={dpi}
                    onChange={(e) => setDpi(Math.max(50, Math.min(600, Number(e.target.value) || 200)))}
                  ...
                  <label className="ml-4 flex items-center gap-2 text-sm">
                    <input
                      ...
                      className="rounded"
                    />
                    <span className="text-sm text-gray-600">First page only</span>
                  </label>
                </div>
              </div>
            </div>
          ...
          <div className="mt-6 text-sm text-gray-500">
            <p>Accepted: PDF files only. Max file size: depends on your backend. Poppler must be installed on the server.</p>
          </div>
        ...
        <div className="mt-8 text-xs text-gray-400">
          <p>
            Tip: If you run into CORS or network errors, make sure your Django server allows requests from your frontend
            origin and that the endpoint is correct.
          </p>
        </div>
      </div>
    </div>
